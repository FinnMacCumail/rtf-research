...
### Added
- **Phase 5: a multi-turn session harness — one wrong scope produced four wrong answers - Sep 2026**: Every v5 question starts cold, so the benchmark is structurally blind to failures caused by turns sharing a conversation. A new harness runs 11 turns down one accumulating thread, using v5-verbatim questions so matched turns score against the existing references ([ollamaDeepAgents](https://github.com/FinnMacCumail/ollamaDeepAgents), `c5f534b`)
  - **First run scored 0.750 where the same questions scored 1.0 cold.** Ten scored turns, 39 tool calls, **36 min wall clock** (26 min of agent turns; the remainder is the `gpt-oss:20b` scorer between turns, so it is not directly comparable to v5's 271.6 s/question), peak context 46,376 of 131,072, compaction never fired, decode 14.83 → 12.62 t/s. Three turns that each scored **1.0 in the full v5 run** came back at 0.5
  - **All three trace to one origin turn.** Turns 1–3 were scoped to site `HVL-SEA-DC1` and turn 3 fetched every device at `site_id: 25`. Turn 4 then asked a **tenant**-scoped question and answered *"there are **6** PDUs **at the site**"* from that inherited list — **never issuing a tenant query**. Six PDUs / 48 outlets against a true 12 / 96; turns 5, 6 and 8 inherited the wrong denominator, turn 6 with **zero** tool calls
  - **Reordering the same 11 questions recovered it: 0.750 → 0.950**, on 38 tool calls against 39 — three recoveries, seven ties, **zero regressions**. Turn 1 now resolves `tenancy.tenant` → id 14, calls `netbox_get_objects(dcim.device, tenant_id=14)`, and answers **12 PDUs, 8 outlets each**. That question scored **0.0 in v5 and 0.0 in the first session**, so it is correct here for the first time in any condition
  - **The load-bearing evidence is a turn that did nothing.** The same question is answered with zero tool calls in both runs: contaminated 0.5, clean **1.0**. Identical behaviour, opposite outcome — **context reuse is not the defect, reusing a wrong population is**. Cold, this defect costs one question; in session it cost four. Contamination also has a direction: with site questions moved to turns 8–10 they still scored 1.0 and re-queried rather than inheriting
  - **Question order also decides GraphQL routing.** The site-first run made **zero** GraphQL calls; the tenant-first run used it on three turns. The skill was never being declined — a cold cross-model question triggers it, and none arrived cold in the first ordering. It then routed badly, burning four malformed queries before an MCP fallback produced the right answer
  - **A fourth projection miss, the first in the other direction**: predicted 3.5–4.5 h, actual **36 min** (~6× wrong) where the previous three were optimistic. Sized from the 90-question run's 75,571-token tail when one thread of related questions peaks at 46,376 and reuses almost everything
  - **Limits stated**: one run per ordering; three improved / seven tied / none worse is **p ≈ 0.25** on a sign test, so the weight is on the observed `tenant_id` query, not the tally. One turn scored 0.5 in both orders despite an answer matching v5's 1.0-scoring answer — probably judge noise, left as measured
  - **Docs**: ADR-0038 gains a *Negative / limitations* entry on multi-turn contamination; Phase 5 → Local Frontier Inference gains *The session harness: one wrong scope, four wrong answers*
- **Phase 5: the full 90-question benchmark run locally — 6 h 47 m, correctness 0.906 - Sep 2026**: Every runtime figure for local inference until now was extrapolated from 3 or 12 questions. The full harness has now been executed end to end ([ollamaDeepAgents](https://github.com/FinnMacCumail/ollamaDeepAgents))
  - **6 h 47 m at 271.6 s/question**, correctness **0.906** (simple 0.917 / medium 0.933 / advanced 0.867), completeness 0.944, entity coverage 0.866, 5.52 tool calls, **0 errors**, **zero truncations**, **zero overflows**, peak context **75,571** of 131,072. Decode held at a 13.47 t/s median across 507 requests, falling only to 10.25 at peak context
  - **Level with deepseek-v4-flash, and this benchmark cannot separate them.** Against the **re-scored** cloud figures on the same 90 items — flash 0.911, kimi 0.928, pro 0.944, qwen3.5:397b 0.817 — the local model is **0.5pp below flash**. ADR-0037 records that the set resolves ~9pp and above and cannot resolve ~3pp, so the claim is **indistinguishable from flash here**, not "matches" and not "beats". The **8.9pp over qwen3.5:397b-cloud** *is* within resolution. The comparison must use re-scored numbers: three wording fixes raised flash 0.906 → 0.911, kimi 0.911 → 0.928, pro 0.933 → 0.944, and the local run came after them
  - **My projection missed again, and the reason generalises.** From a 12-question sample I predicted 3.2–4.5 h; the answer was 6.78 h — **optimistic by 1.5–2.1×**. That sample peaked at 22,532 tokens of context where the full set reached 75,571. It was stratified across tiers and *still* missed the expensive tail: **a subset representative of difficulty is not thereby representative of cost**
  - **Three of the seven failures are one bug.** On three questions the agent searched for "Halvorsen Logistics" as a circuits *provider*, found nothing, and answered **"Halvorsen Logistics was not found in NetBox"** — confidently, after 2–5 genuine tool calls, about the seeded tenant 57 of the 90 questions depend on. Every other question found it. A scoping failure producing an authoritative denial rather than an error — the same shape as the silent truncation recorded earlier, and a fixable routing bug rather than three capability misses
  - **Attribution caveat**: this run differs from the ~15.8 h figure in **four** ways (KV in VRAM, `reasoning_effort="low"`, `max_tokens` 8192, model `profile` set). The 2.33× belongs to the configuration as a whole; the clean per-flag figure for removing `--no-kv-offload` remains the **1.87×/2.02×** from the three-question A/B where nothing else varied
  - **Docs**: ADR-0038's *Negative / limitations* gains the measured runtime and the cloud comparison with the n=1 caveat struck in place; Phase 5 → Local Frontier Inference gains a *The full harness, finally run* section
- **Phase 5: the KV-in-VRAM result replicated, and the cold first turn proven - Sep 2026**: A second run of the same three NetBox questions, launched from the **committed `serve_qwen4exp.sh`** rather than a hand-typed command line — so the artefact in use was what got validated ([ollamaDeepAgents](https://github.com/FinnMacCumail/ollamaDeepAgents))
  - **3/3 correct again**, one accumulating thread, peak context 9.5k / 10.9k / 19.1k, all traces correctly labelled `qwen3.8-flash-next-UD-Q4_K_XL`. **Q2 — the warm comparison, 2 LLM calls in both runs — came in at 76.9 s against the original 171.2 s (2.23×), beating the first VRAM run's 91.6 s.** Q3 1.61×
  - **The cold first turn is now demonstrated, not argued.** Per-call timings inside Q1: **108.6 s** at 8,743 prompt tokens, then **20.4 s** at 8,958 and **23.7 s** at 9,534 — *more* tokens at one-fifth the time once the prefix cache fills, with 152.7 s of the 155.9 s wall server-side. **A benchmark whose first question follows a restart is measuring cache warm-up**, and should discard it rather than average it in
  - **Variance is the bigger caution, not speed.** The same Q3 under the same configuration took **9 LLM calls in one run and 14 in the other**, while tool execution accounted for just **5.4 s of 413.9 s** — the cost is round-trips, not tools. Both runs issued **four `netbox_graphql_schema` introspections**, and a single advanced-tier timing is one sample, not a figure. *(Corrected Sep 2026: this originally read "re-learning the schema mid-query", framing mandated behaviour as waste. The skill requires introspection before querying an unfamiliar type, and the tool caches the schema per process. Across the full 90-question run it is 44 introspections against 65 queries — ~0.95% of runtime — and one introspection once replaced 19 MCP calls.)*
  - **Still not re-run**: the 12-question benchmark. The `~19.4 h` / `~15.8 h` extrapolations remain lower bounds on speed rather than being replaced by a modelled figure
